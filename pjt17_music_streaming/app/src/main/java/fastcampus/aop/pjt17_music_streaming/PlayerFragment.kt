...
import fastcampus.aop.pjt17_music_streaming.databinding.FragmentPlayerBinding
import fastcampus.aop.pjt17_music_streaming.service.MusicDto
import fastcampus.aop.pjt17_music_streaming.service.MusicService
...
class PlayerFragment: Fragment(R.layout.fragment_player) {
    ...
    private fun updateSeek() {
        ...
        // 중복될 경우 이전 Runnable 삭제
        view?.removeCallbacks(updateSeekRunnable)

        // 재생 중이라면 해당 함수를 1초 뒤에 한 번 더 호출
                ...
                .enqueue(object : Callback<MusicDto> {
                    override fun onResponse(call: Call<MusicDto>, response: Response<MusicDto>) {
                        if (!response.isSuccessful) {
                            Toast.makeText(context, "음악 목록을 불러오는 데 실패했습니다.", Toast.LENGTH_SHORT).show()
                            return
                        }
