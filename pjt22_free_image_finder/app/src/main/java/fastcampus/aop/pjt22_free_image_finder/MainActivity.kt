package fastcampus.aop.pjt22_free_image_finder
...
import fastcampus.aop.pjt22_free_image_finder.data.Repository
import fastcampus.aop.pjt22_free_image_finder.data.models.PhotoResponse
import fastcampus.aop.pjt22_free_image_finder.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    private fun bindViews() {
        ...
        (binding.recyclerView.adapter as? PhotoAdapter)?.onClickPhoto = { photoResponse ->
            showDownloadPhotoConfirmationDialog(photoResponse)
        }
    }

    private fun showDownloadPhotoConfirmationDialog(photoResponse: PhotoResponse) {
        AlertDialog.Builder(this)
            .setMessage("이 사진을 저장하시겠습니까?")
            .setPositiveButton("저장") { dialog, _ ->
                downloadPhoto(photoResponse.urls?.full)
                dialog.dismiss()
            }
        ...
        val imageCollectionUri =
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                MediaStore.Images.Media.getContentUri(
                    MediaStore.VOLUME_EXTERNAL_PRIMARY    // 읽고 쓰기 가능
                )
            } else {
